...
SELECT gs1.sidename AS team, 
		round(avg(sa.squad_average-ta.team_average),0) as average_difference_squad_minus_team
...
(SELECT id FROM game WHERE LOWER(name) LIKE 'ps7%' OR LOWER(notes) LIKE '%ps7%')
   AND gs1.size = 3
   AND gs1.id != gs2.id
  GROUP BY gs1.sidename
  ORDER BY average_difference_squad_minus_team DESC